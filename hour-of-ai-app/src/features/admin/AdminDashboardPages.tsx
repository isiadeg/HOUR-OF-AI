...
const AlertItem = ({
  ...
  return <div className={`flex items-center gap-3 p-3.5 rounded-xl border ${bg}`}>
      ...
      {action && <button onClick={onAction} className="text-xs font-bold text-[#0B6B3A] hover:underline shrink-0 whitespace-nowrap">{action}</button>}
    </div>;
};
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white border border-[#d1e8db] rounded-2xl p-6 shadow-sm">
          ...
          <div className="space-y-2.5">
            {alerts.map((a, i) => <AlertItem key={i} msg={a.msg} type={a.type} action={a.action} onAction={() => onNavigate(a.page)} />)}
          </div>
        </div>
...
const ClassManagementPage = () => {
  ...
  const [section, setSection] = useState<'suggested' | 'active' | 'create'>(() =>
    location.pathname.endsWith('/create') ? 'create' : 'suggested'
  );
  ...
  return <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-gray-900 mt-0.5">{sg.timeSlot} <span className="text-gray-400 font-normal">({sg.timezone})</span></p>
                    </div>
                    <div>
      ...
      {section === 'active' && <div className="bg-white border border-[#d1e8db] rounded-2xl overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-[#f7fbf9] border-b border-[#e6f4ed]">
                <tr>
      ...
      {section === 'create' && <div className="bg-white border border-[#d1e8db] rounded-2xl p-6 shadow-sm max-w-2xl">
          ...
          options: VOLUNTEERS.filter(v => v.availabilityStatus === 'Available').map(v => v.name)
            ...
            [field.key]: e.target.value
              ...
              <div className="sm:col-span-2 p-3 bg-[#f0f9f4] rounded-xl border border-[#d1e8db] text-xs text-[#0B6B3A] font-medium flex items-center gap-2">
                <Shield size={13} />
                <span>Minimum 2 volunteers required · Volunteers cannot be double-booked · Availability will be validated on submit</span>
              </div>
...
const CommunicationsPage = () => {
  ...
  const audienceOptions = ['All Users', 'All Volunteers', 'Parents', 'Community Hubs', 'Specific Class'];
  return <div className="space-y-6">
      <PageHeading label="Broadcast & Notify" title="Communications" sub="Send announcements and messages to your community." />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-[#d1e8db] rounded-2xl p-6 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-5">New Announcement</h3>
          {sent ? <div className="flex items-center gap-3 p-4 bg-[#e6f4ed] rounded-xl">
              <CheckCircle size={18} className="text-[#0B6B3A]" />
              <p className="text-sm font-semibold text-[#0B6B3A]">Announcement sent successfully.</p>
            </div> : <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Title</label>
                <input value={title} onChange={e => setTitle(e.target.value)} placeholder="Announcement title…" className="w-full border border-[#d1e8db] rounded-xl px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:border-[#0B6B3A]" />
              </div>
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Message</label>
                <textarea value={message} onChange={e => setMessage(e.target.value)} rows={5} placeholder="Write your message…" className="w-full border border-[#d1e8db] rounded-xl px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:border-[#0B6B3A] resize-none" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 mb-1.5">Schedule Send (optional)</label>
                <input type="datetime-local" value={scheduled} onChange={e => setScheduled(e.target.value)} className="w-full border border-[#d1e8db] rounded-xl px-3 py-2.5 text-sm text-gray-700 focus:outline-none focus:border-[#0B6B3A]" />
              </div>
              <button onClick={() => {
            if (title && message && audience) setSent(true);
                ...
                <Send size={14} /><span>{scheduled ? 'Schedule Announcement' : 'Send Now'}</span>
              </button>
            </div>}
        </div>

        <div className="bg-white border border-[#d1e8db] rounded-2xl p-6 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-5">Announcement History</h3>
          <div className="space-y-3">
            {ANNOUNCEMENTS.map(a => <div key={a.id} className="border border-[#e6f4ed] rounded-xl p-4 hover:border-[#c3e0cf] transition-colors">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <p className="text-sm font-semibold text-gray-900">{a.title}</p>
                    <p className="text-xs text-gray-400 mt-1">{a.audience} · {a.sentAt}</p>
                  </div>
                  <Badge variant={a.status === 'Sent' ? 'green' : a.status === 'Scheduled' ? 'amber' : 'gray'}>{a.status}</Badge>
                </div>
              </div>)}
          </div>
        </div>
...
export type AdminPageId = 'overview' | 'students' | 'volunteers' | 'classes' | 'programmes' | 'events' | 'communications' | 'sponsorship' | 'reports' | 'settings';
export const AdminPage = ({
  ...
  const map: Record<AdminPageId, React.ReactElement> = {
    ...
    events: <EventsPage />,
    communications: <AdminCommunicationConsole />,
